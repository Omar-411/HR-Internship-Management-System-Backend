...
export const getSupervisorDashboard = async (user) => {
  ...
  const activities = tasks
    ...
    .map((t) => {
      const assignee = teamMembers.find(
        (m) => String(m._id) === String(t.assignedTo),
      );
  ...
  const completionByUser = teamMembers.map((member) => {
    const completedCount = tasks.filter(
      (t) => String(t.assignedTo) === String(member._id) && t.status === "Done",
    ).length;
    return {
...
export const getAdminDashboard = async (user) => {
  ...
  const totalExpectedAll = Array.from(statsMap.values()).reduce(
  ...
  const distributionMap = { Present: 0, Late: 0, Absent: 0, "On Leave": 0 };
  ...
  dailyAttendance.forEach((a) => {
    ...
    if (status === "present") {
      distributionMap["Present"] += 1;
    } else if (status === "late") {
      distributionMap["Late"] += 1;
  ...
  const attendanceDistribution = Object.entries(distributionMap).map(
...
export const getDashboardStats = async (user) => {
  ...
  const weeklyCompletionStats = Array.from({ length: 6 }, () => ({
    totalDays: 0,
    count: 0,
  }));
  ...
  tasks.forEach((t) => {
    ...
    if (t.dueDate) {
      ...
      if (due < now && s !== "done" && s !== "approved") {
        if (s === "to do" || s === "todo" || s === "backlog")
          overdueCounts.todo++;
        else if (s === "in progress" || s === "in_progress")
          overdueCounts.inProgress++;
  ...
  const avgCompletionTime = weeklyCompletionStats.map((stat, idx) => {
    return {
      ...
      avgDays:
        stat.count > 0
          ? parseFloat((stat.totalDays / stat.count).toFixed(1))
          : 0,
    };
  });
